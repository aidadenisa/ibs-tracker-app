...
describe('Day Report', function() { 
  ...
  it('shows the current week correctly', function() {
    const date = new Date();
    const days = getWeekDaysByDate(date);

    expect(days.length).to.eq(7);

    for(let i=0; i < days.length; i++) {
      cy.get('.week-calendar .week-calendar__day').contains((new Date(days[i])).getDate());
      cy.get('.week-calendar .week-calendar__day-name').contains(getFormattedDayName(days[i]));
    }
    ...
    cy.get('div.week-calendar__day-name:first').should('not.eq', getFormattedDayName(dayBeforeTheWeek.toISOString()));
    cy.get('div.week-calendar__day-name:last').should('not.eq', getFormattedDayName(dayAfterTheWeek.toISOString()));
